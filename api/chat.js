...
// ─────────────────────────────────────────────────────────────────────────────
// TRI-VECTOR VALUE FIELD
// V_Agency(주체성) × V_Morning(아침형) × V_Musical(음악적사랑)
// V1-V7 user resonance data → mapped to 3-vector 9-dimensional attractor
// ─────────────────────────────────────────────────────────────────────────────

// Default baseline values (ARHA's character floor — active even with no user data)
...
// ─────────────────────────────────────────────────────────────────────────────
// L0 Core Anchors — v3.1 (mirror of services/personaRegistry.ts L0_CORE_ANCHORS)
// Per-persona immutable identity layer. See Doc 10 §4.4.
// ─────────────────────────────────────────────────────────────────────────────
...
function getL0CoreAnchor(personaId) {
...
function heuristicDecompose(goal) {
  if (!goal || typeof goal !== 'string') {
    return { sub_goal_count: 1, domains: [], complexity: 0.1, length_factor: 0 };
  }
...
function maxSubsForMode(mode) {
  return mode === 'triangle' ? 2 : mode === 'square' ? 3 : mode === 'pentagon' ? 4 : 5;
}

// ─────────────────────────────────────────────────────────────────────────────
// buildAnchorPromptBlock — Τ_PromptAnchor (Doc 10 §5.1)
// Formats L0/L1/L2 hierarchy as system prompt section.
// Requires pre-computed triVector (from computeTriVectorFieldData below).
// ─────────────────────────────────────────────────────────────────────────────
const FX_TO_L1 = {
...
function buildAnchorPromptBlock(personaId, goal, triData) {
  ...
  const L1_main = L1Keys.map(key => {
    const vec = key === 'V_Agency'  ? triData.agency
              : key === 'V_Morning' ? triData.morning
              : triData.musical;
...
function computeTriVectorFieldData(personaValueChain) {
  ...
  // Apply V1-V7 user resonance → modulate tri-vector components
...
function buildSystemPromptV2(triggers, prevState, kappa, personaValueChain, personaPrompt, proData, situation, userMemoryBlock, personaId, goal) {
  ...
  // ③ Tri-vector value field + L0/L1/L2 anchor hierarchy (Doc 10 §5.1 Τ_PromptAnchor)
...
// ─────────────────────────────────────────────────────────────────────────────
// Named exports — for Phase 1 verification tests only (not used by Vercel runtime)
// ─────────────────────────────────────────────────────────────────────────────
export {
  heuristicDecompose,
  pickAnchorMode,
