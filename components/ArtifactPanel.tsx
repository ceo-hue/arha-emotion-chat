  ...
  return (
    <div className="relative rounded-xl overflow-hidden border border-white/10 mt-2">
      <div className="flex items-center justify-between px-3 py-1.5 bg-white/5 border-b border-white/10">
        <span className="text-[9px] font-black uppercase tracking-widest text-white/40">{lang}</span>
        <button
          onClick={copy}
          className="flex items-center gap-1 text-[9px] text-white/30 hover:text-white/70 transition-all"
        >
          {copied ? <Check size={10} className="text-emerald-400" /> : <Copy size={10} />}
          {copied ? 'Copied' : 'Copy'}
        </button>
      </div>
      <pre className="px-4 py-3 text-[11px] leading-relaxed text-white/75 overflow-x-auto font-mono bg-black/30">
        <code>{content}</code>
      </pre>
    </div>
  );
    ...
    <div className="flex flex-col h-full">
      ...
      <header className="h-12 md:h-16 px-4 md:px-5 border-b border-white/10 bg-white/5 backdrop-blur-xl flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <h3 className="text-[12px] font-bold text-white/80 truncate">{artifact.title}</h3>
        </div>
        <button
          onClick={onClose}
          className="w-8 h-8 rounded-xl flex items-center justify-center text-white/40 hover:text-white hover:bg-white/10 active:bg-white/20 transition-all shrink-0 ml-2"
        >
          <X size={16} />
        </button>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-5 space-y-4 scroll-hide">
        {artifact.sections.map((section, i) => (
          <div key={i} className="space-y-2">
            {section.heading && (
              <h4 className="text-[10px] font-black uppercase tracking-wider text-white/40 border-b border-white/10 pb-1">
                {section.heading}
              </h4>
            )}
            {section.body && (
        ...
        <div className="flex items-center gap-2 pt-2 border-t border-white/10">
          <span className="text-[9px] font-black uppercase tracking-widest text-white/20">P_MODE</span>
          <div className="w-1.5 h-1.5 rounded-full bg-violet-500/60" />
          <span className="text-[9px] text-white/20">PROMETHEUS Active</span>
        </div>
      </div>
    </div>
  );
